...
func (c *tcpClient) TryRunOnce(ctx context.Context, outputToStdout bool) (bool, string, error) {
	...
	if dl, ok := ctx.Deadline(); ok { if d := time.Until(dl); d > 0 { deadline = d } }
	...
	for port := start; port <= end; port++ {
		...
		if !ping(addr, deadline) { continue }
		// connect for request
		conn, err := net.DialTimeout("tcp", addr, deadline)
		if err != nil { continue }
		w := bufio.NewWriter(conn)
		if outputToStdout { _, err = w.WriteString("STDOUT\n") } else { _, err = w.WriteString("CLIPBOARD\n") }
		if err != nil { conn.Close(); return true, "", err }
		if err := w.Flush(); err != nil { conn.Close(); return true, "", err }
