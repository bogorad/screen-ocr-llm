...
func TestServerClientRoundTrip(t *testing.T) {
	...
	go func() {
		...
		if err != nil { t.Errorf("client: %v", err) }
		if !delegated { t.Errorf("expected delegation") }
	}()

	// server accept and respond
	conn, err := srv.Next(ctx)
	if err != nil { t.Fatalf("next: %v", err) }
	if !conn.Request().OutputToStdout { t.Errorf("expected stdout request") }
	if err := conn.RespondSuccess("ok"); err != nil { t.Fatalf("respond: %v", err) }
	<-delegatedCh
}
