...
func wndProc(hwnd syscall.Handle, msg uint32, wParam, lParam uintptr) uintptr {
	...
	case WM_TIMER:
		// Timer expired, close window
		log.Printf("Popup: Timer expired, closing window")
...
func createAndShowPopup(text string) error {
	...
	// Set timer to close window after 3 seconds
	timerResult, _, _ := procSetTimer.Call(hwnd, 1, 3000, 0)
	log.Printf("Popup: Set 3-second timer, result: %d", timerResult)

	// Simple message loop with timeout
	startTime := time.Now()
	var msg MSG
	for time.Since(startTime) < 5*time.Second { // Max 5 seconds to prevent hanging
